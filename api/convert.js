(() => {
  try{
    ...
    var g=i=>parts[i]||"", ok=v=>v==="yes"||v==="no";
    var zv=g(0), baby=g(1), jockey=g(2), armor=g(3), ap=g(4), ae=g(5), at=g(6), loot=g(7), we=g(8), hand=g(9), pk=g(10);
    if(!ok(zv)||!ok(baby)||!ok(jockey)||!ok(armor)) throw 1;
    if(armor==="yes"){ if(!["1","2","3","4"].includes(ap)||!["0","1","2","3","4"].includes(ae)||!["l","co","ch","i","g","d"].includes(at)) throw 1; } else { ap="0"; ae="0"; at="x"; }
    ...
    return "Your naturally spawned zombie rarity in max local difficulty is 1/" + denom + " or " + pct + "%";
  }catch(e){
    return "Wrong input. Use: !zr yes/no◆yes/no◆... See twitch.tv/hassannm7";
  }
})()
