(() => {
  try{
    var raw = (q||"").replace(/\uFEFF/g,"").trim();
    if(!raw) throw 1;
    // allow multiple separators: ◆ or | or ; or /
    var parts = raw.split(/\s*(?:◆|\||;|\/)\s*/).map(s=>s.trim().toLowerCase());
    if(parts.length<11) throw 1;
    var g=i=>parts[i]||"", yn=v=>["yes","y","no","n"].includes(v);
    var normYN=v=>{
      if(v==="y") return "yes"; if(v==="n") return "no"; return v;
    };
    var zv=normYN(g(0)), baby=normYN(g(1)), jockey=normYN(g(2)), armor=normYN(g(3));
    var ap=g(4), ae=g(5), at=g(6), loot=g(7), we=normYN(g(8)), hand=g(9), pk=g(10);
    if(!yn(zv)||!yn(baby)||!yn(jockey)||!yn(armor)) throw 1;
    if(armor==="yes"){
    ...
    if(loot!=="none"&&loot!=="can pick up"){ if(!yn(we)) throw 1; we = (we==="y"?"yes":we); } else we="no";
    ...
    var pieces=parseInt(ap,10), enchCount=parseInt(ae,10);
    if(armor==="yes"){
    ...
    return "Wrong input. Use: !zr yes/no◆yes/no◆... or separate with | ; / — see twitch.tv/hassannm7";
  }
})()
